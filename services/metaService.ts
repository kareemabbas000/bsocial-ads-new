...
export const fetchPlacementBreakdown = async (
  ...
  const promises = adAccountIds.map(async (accId) => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/${GRAPH_API_VERSION}/${formattedId}/insights?breakdowns=publisher_platform,platform_position&fields=spend,impressions,clicks,actions,action_values&limit=100&access_token=${token}${dateParam}${filtering}`
        );
        const data = await response.json();
        return data.data || [];
      } catch (e) { return []; }

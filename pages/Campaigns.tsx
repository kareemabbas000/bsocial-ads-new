...
const Campaigns: React.FC<CampaignsProps> = ({ token, accountIds, datePreset, theme, filter, userConfig, refreshInterval = 10, refreshTrigger = 0 }) => {
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            setDrafts({});
            loadData(false);
            setNotification({ type: 'success', message: "Changes published successfully!" });
            setTimeout(() => setNotification(null), 3000);
        } catch (e: any) {
            console.error(e);
            setNotification({ type: 'error', message: "Error publishing changes. Check console." });
    ...
    console.log('[Campaigns] Rendering with notification type:', notification?.type);
        ...
        <div className="flex flex-col h-full relative">
            ...
            <div className="flex flex-col space-y-4 mb-4">
                ...
                <div className="flex items-center space-x-2 text-sm overflow-x-auto pb-2 scrollbar-hide">
                    ...
                    {activeLevel !== 'CAMPAIGN' && <ChevronRight size={14} className="text-slate-400" />}
                    {selectedCampaignId && <button onClick={() => { setActiveLevel('ADSET'); setSelectedAdSetId(null); }} className={`font-semibold flex items-center transition-colors ${activeLevel === 'ADSET' ? 'text-brand-500' : 'text-slate-500'}`}>Campaign Checks</button>}
                    {(activeLevel === 'AD' || selectedAdSetId) && <ChevronRight size={14} className="text-slate-400" />}
                    {selectedAdSetId && <span className={`font-semibold ${activeLevel === 'AD' ? 'text-brand-500' : 'text-slate-500'}`}>Ad Creative</span>}
                </div>
            ...
            {/* Bottom Panel, Drawers, etc would go here but we assume they are covered or handled by common layouts/modals if not explicitly in this snippet. Preserving main table structure. */}
        </div>
    );
};
